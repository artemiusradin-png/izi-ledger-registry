...
function normalizeRelatedSlugs(works) {
  const slugByFileName = new Map(works.map((work) => [work.fileName, work.slug]));
  const knownSlugs = new Set(works.map((work) => work.slug));

  return works.map((work) => {
    const relatedRefs = Array.isArray(work.relatedRefs) ? work.relatedRefs : [];
    const relatedSlugs = relatedRefs
      .map((ref) => slugByFileName.get(ref) ?? (knownSlugs.has(ref) ? ref : null))
      .filter(Boolean);

    if (relatedSlugs.length === 0) {
      return { ...work, relatedRefs: undefined };
    }

    return { ...work, relatedRefs: undefined, relatedSlugs };
  });
}
...
function main() {
  ...
  for (const htmlFile of htmlFiles) {
    ...
    works.push({
      ...
      relatedRefs: Array.isArray(override.related) ? override.related.filter((value) => typeof value === "string") : []
    });
  }
